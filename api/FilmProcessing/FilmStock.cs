...
/// <summary>
/// A named film look: a display name plus the ordered filter chain that defines
/// it. Stocks are data — the maths lives in the filters — so adding a "roll" is a
/// single row in <see cref="FilmStocks.Defs"/> (or a drop-in <c>.cube</c> LUT).
/// </summary>
public sealed record FilmStock(string Id, string DisplayName, IReadOnlyList<IImageFilter> Chain);

/// <summary>
/// The catalogue of selectable film looks — the popular colour-negative, slide,
/// and black-and-white stocks. Each row is a set of grade/tone/halation/grain/
/// vignette parameters; the chain is built in a fixed order:
/// colour grade → tone curve → halation → grain → vignette.
///
/// If <c>FilmProcessing/Luts/{id}.cube</c> exists it replaces the parametric grade
/// with a real 3D LUT, so you can ship exact emulations per stock over time
...
public static class FilmStocks
{
    ...
    // grade = lift/gamma/gain/saturation; tone = blackLift/contrast;
    // hal = halation threshold (null = none)/intensity/radius; grain; vignette.
    ...
    // Ordered so the client picker reads sensibly (Kodak → Fuji → CineStill → B&W).
    ...
    /// <summary>Ids/names for a client picker, without building the chains.</summary>
    ...
    private static FilmStock Build(string id)
    {
        ...
        // Colour: a real LUT if we have one, else the parametric grade.
        ...
        // Seed grain per-stock so a photo always develops identically — a *stable*
        // hash, since string.GetHashCode() is randomised per process run.
    ...
    // Deterministic FNV-1a so grain is reproducible across restarts.
